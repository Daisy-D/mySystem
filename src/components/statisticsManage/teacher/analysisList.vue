...
<template>
  <div class="wrap">
    <div class="title_wrap">
      <div class="title_style">教师工作量统计</div>
    ...
    <Modal v-model="isExport" :mask-closable="false" :scrollable="false" class-name="subjectModal">
  ...
  methods: {
    ...
    */
    addSubjectExport(){
      ...
      window.location.href =
        nozzle.statisticsCtrl.exportTeaStatistics +
        "?orderWay=" +
    ...
    */
    loadinfoDataAjax(_data) {
      ...
      window.util.ajax.get(nozzle.statisticsCtrl.teaStatistics, {
          params: _data
        }).then(function(res) {
          _this.$store.state.isLoadingShow=false;
          var ajaxInterface = window.util.checkAjaxJson(res);
          ajaxInterface.thenSuccess(function(json) {
            ...
            }).autoRun("login", "error", "teacherAnalysisList");
        }).catch(function(error) {
          window.util.checkAjaxError(error).autoRun("teacherAnalysisList");
        });
    }
  },
  mounted() {
