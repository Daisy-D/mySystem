        ...
        <Form ref="ruleBaseInfo" :model="ruleBaseInfo" :rules="ruleCheck"  label-position="right" :label-width="100">
            ...
            <Row>
                <Form-item label="用户名：" prop="ruleUserName">
                    <Input class="inputWidth" :disabled="isEditSta" v-model="ruleBaseInfo.ruleUserName" placeholder="6-20位，字母开头，包含字母、数字或下划线"></Input>
                </Form-item>
            </Row>
            <Row>
                <Form-item label="密码：" prop="rulePassword">
                    ...
                    <!--<span style="color: #ccc;margin-left: 10px;">如果不设置密码，则默认密码为：SPOC123</span>-->
                </Form-item>
                <p style="color: #ccc;margin-left: 10px;"></p>
            </Row>
            <Row>
                <Form-item label="姓名：" prop="ruleName">
                    <Input class="inputWidth" :disabled="isEditSta" v-model="ruleBaseInfo.ruleName" placeholder=""></Input>
                </Form-item>
            </Row>
            <Row>
                <Form-item label="手机号码：" prop="rulePhone">
                    <Input class="inputWidth" v-model="ruleBaseInfo.rulePhone" placeholder=""></Input>
                </Form-item>
            </Row>
            <Row>
                <Form-item label="学号：" prop="ruleNum">
                    <Input class="inputWidth" v-model="ruleBaseInfo.ruleNum" placeholder=""></Input>
                </Form-item>
            </Row>
            <Row>
                <FormItem label="学校：" prop="ruleSchool">
                    <!--<Select class="inputWidth" filterable v-model="ruleBaseInfo.ruleSchool" placeholder="" @on-change="">-->
            ...
            <Row>
                <FormItem label="专业：" prop="ruleMajor">
                    <!--<Select class="inputWidth" filterable v-model="ruleBaseInfo.ruleMajor" placeholder="">-->
                        <!--<Option v-for="item in majorList" :value="item.value" :key="item.value">{{item.label}}</Option>-->
                    <!--</Select>-->
                    <select-drop :root-msg="rootMsg" v-model="ruleBaseInfo.ruleMajor" :type=2 style="width: 280px;"></select-drop>
                </FormItem>
            </Row>
            <Row>
                <FormItem label="年级：" prop="ruleGrade">
                    <!--<Select class="inputWidth" filterable v-model="ruleBaseInfo.ruleGrade" placeholder="">-->
                        <!--<Option v-for="item in gradeList" :value="item.value" :key="item.value">{{item.label}}</Option>-->
                    <!--</Select>-->
                    <select-drop :root-msg="rootMsg" v-model="ruleBaseInfo.ruleGrade" :type=3 style="width: 280px;"></select-drop>
                </FormItem>
            </Row>
            <Row>
                <Form-item label="班级：" prop="ruleClass">
                    <Input class="inputWidth" v-model="ruleBaseInfo.ruleClass" placeholder=""></Input>
                </Form-item>
            </Row>
            <Row>
                <FormItem label="角色：" prop="ruleRole">
                    <Select disabled class="inputWidth" v-model="ruleBaseInfo.ruleRole" placeholder="">
            ...
            <Row>
                <Form-item style="text-align: right;margin-top: 30px;">
                    <Button type="ghost" :class="isEditSta?'submitBtn_edit':'submitBtn'" style="margin-right: 10px;" @click="submitBtnCancelFunc">取消</Button>
                    <Button type="primary" :class="isEditSta?'submitBtn_edit':'submitBtn'" @click="handleSubmit('ruleBaseInfo')">提交</Button>
                </Form-item>
            </Row>
        </Form>
...
//                var _value = value.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
            ...
            const checkPhone = (rule, value, callback) => {
                ...
                value = value.replace(/\s\s*/, '')
                this.ruleBaseInfo.rulePhone = value;
                var regResult = reg.test(value)
                if (value.length > 0 && !regResult) {
            ...
            const checkNum = (rule, value, callback) => {
                ...
                var regResult = reg.test(value)
                 if (value.length>0 && !regResult) {
                    callback(new Error('学号格式不正确，包含5-20位数字或字母'));
                 }
                callback()
            };
            ...
            const checkClass = (rule, value, callback) => {
                ...
                value = value.replace(/\s\s*/, '');
                this.ruleBaseInfo.ruleClass = value;
                var regResult = reg.test(value)
                if (value.length>0 && !regResult) {
                    callback(new Error('班级格式不正确，包含小于50位数字、字母或汉字'));
                }
                callback()
            };
            return{
...
//                    ruleSchool:[
//                        { validator: checkSchool, trigger: 'change'}
//                    ],
//                    ruleMajor:[
...
//                    ],
                    ruleClass:[
                        { validator: checkClass, trigger: 'change'}
                    ],
//                    ruleRole:[
//                        { validator: checkClass, trigger: 'change'}
//                    ],
        ...
        methods:{
            ...
            getSchoolListFunc(){
                ...
                function callbackDeal(json) {
                    ...
                    _this.rootMsg.$emit('schoolList',_this.schoolList);
                }
            },
            //获取专业列表
            getMajorListFunc(){
                ...
                function callbackDeal(json) {
                    ...
                    if(json.status=="success"){
                        ...
                        _this.rootMsg.$emit('majorList',_this.majorList);
                    }
                }
            },
            //获取年级列表
            getGradeListFunc(){
                ...
                function callbackDeal(json) {
                    ...
                    if(json.status=="success"){
                        ...
                        _this.rootMsg.$emit('gradeList',_this.gradeList);
                    }
                }
            },
            //获取职称列表
            getjobTitleListFunc() {
                ...
                function callbackDeal(json) {
                    ...
                    if (json.status == "success") {
                        ...
                        _this.rootMsg.$emit('jobTitleList',_this.jobTitleList);
                    }
                }
            },
            //校验提交
...
//                return
                this.rootMsg.publicAjax('post',baseUrl.userManage.addUser,{
                        ...
                        professionId:Number(this.ruleBaseInfo.ruleMajor),
                        studentNo:this.ruleBaseInfo.ruleNum,
                        gradeId:Number(this.ruleBaseInfo.ruleGrade),
                        jobTitleId:'',
